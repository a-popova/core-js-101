...
function willYouMarryMe(isPositiveAnswer) {
  ...
  if (isPositiveAnswer === true) {
    result = Promise.resolve('Hooray!!! She said "Yes"!');
  } else if (isPositiveAnswer === false) {
    result = Promise.resolve('Oh no, she said "No".');
  } else {
...
function chainPromises(array, action) {
  return array.reduce(
    (chainOfPromises, currentPromise) => chainOfPromises.then(
      (chainResult) => currentPromise.then(
        (promiseResult) => [...chainResult, promiseResult],
        () => chainResult,
      ),
    ), Promise.resolve([]),
